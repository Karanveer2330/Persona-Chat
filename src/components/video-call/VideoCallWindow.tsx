...
import VRMModelSelector from '../avatar/VRMModelSelector';
...
export default function VideoCallWindow({
  ...
  const onAvatarDataUpdate = useCallback((avatarData: any) => {
    ...
    // EXPERIMENTAL: Mirror local data to remote avatar for testing
    setRemoteAvatarData(avatarData);
    
    // Send avatar data to remote user via socket (commented out for testing)
  ...
  const initializeCall = async () => {
    ...
    try {
      ...
      socket.on('userJoinedCall', async ({ userId, userName, socketId }) => {
        joined the call. I am initiator: ${isInitiator}, My userId: ${currentUserId}`);
    ...
    <div className="h-screen w-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex flex-col relative">
      ...
      <div className="fixed top-2 right-2 md:top-4 md:right-4 z-50">
        <Button
          ...
          className={`text-white hover:bg-white/10 px-2 md:px-4 py-1 md:py-2 border backdrop-blur-lg shadow-lg text-xs md:text-sm ${showModelSelector ? 'bg-purple-500/50 border-purple-500' : 'bg-black/30 border-white/20'}`}
        >
          <Menu className="h-3 w-3 md:h-4 md:w-4 mr-1 md:mr-2" />
          <span className="hidden sm:inline font-medium">
            {showModelSelector ? 'Close Models' : 'Models'}
          </span>
        </Button>
      </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row relative">
          ...
          <div className="bg-black/30 backdrop-blur-lg rounded-lg md:rounded-xl p-2 md:p-4 border border-white/10">
            ...
            <div className="space-y-1 md:space-y-2 text-xs">
              ...
              {debugInfo && (
                <div className="flex justify-between text-gray-300">
                  <span>Debug:</span>
                  <span className="text-blue-400 text-xs">{debugInfo}</span>
                </div>
              )}
              
              {/* Additional debug info - Hidden on mobile */}
              <div className="hidden md:block text-xs text-white/40 mt-2 space-y-1">
        ...
        {/* Model Selector Sidebar - Fixed Position - Mobile Responsive */}
        <div 
          className={`fixed top-0 right-0 h-full w-full md:w-96 bg-black/90 backdrop-blur-lg border-l-2 border-purple-500/50 overflow-y-auto shadow-2xl z-40 transform transition-all duration-500 ease-in-out ${
            showModelSelector ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'
          }`}
          style={{ display: showModelSelector ? 'block' : 'none' }}
        >
          <div className="p-3 md:p-4 bg-purple-500/20 border-b border-purple-500/30 flex items-center justify-between">
            <h3 className="text-white font-semibold text-sm md:text-base">VRM Model Selector</h3>
            <Button
              variant="ghost"
              size="sm"
          ...
          <VRMModelSelector 
            ...
            selectedModelUrl={selectedVrmUrl}
          />
        </div>
      </div>
