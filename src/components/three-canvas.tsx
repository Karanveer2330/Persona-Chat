...
interface ThreeCanvasProps {
  ...
  isLandscapeBackgroundEnabled?: boolean;
  toggleLandscapeBackground?: () => void;
}
...
const ThreeCanvas = ({ vrmUrl, isCameraEnabled, enableFaceTracking = true, onAvatarDataUpdate, remoteAvatarData, remoteModelUrl, isDualAvatar = false, localAvatarName = "You", remoteAvatarName = "Remote", callMode = 'shared-environment', onModelChange, isLandscapeBackgroundEnabled: propIsLandscapeBackgroundEnabled, toggleLandscapeBackground: propToggleLandscapeBackground }: ThreeCanvasProps) => {
  ...
  // Landscape background state - use prop if provided, otherwise use internal state
  const [internalLandscapeBackgroundEnabled, setInternalLandscapeBackgroundEnabled] = useState(false);
  const isLandscapeBackgroundEnabled = propIsLandscapeBackgroundEnabled !== undefined ? propIsLandscapeBackgroundEnabled : internalLandscapeBackgroundEnabled;
  ...
  // Create gradient texture matching the UI theme
  const createGradientTexture = useCallback(() => {
    ...
    // Create vertical gradient from dark purplish-blue to lighter blue
    const gradient = ctx.createLinearGradient(0, 0, 0, canvas.height);
    gradient.addColorStop(0, '#1a1a2e'); // Dark purplish-blue at top
    gradient.addColorStop(0.5, '#16213e'); // Medium blue-purple
  ...
  const toggleLandscapeBackground = useCallback(() => {
    if (propToggleLandscapeBackground) {
      propToggleLandscapeBackground();
    } else {
      setInternalLandscapeBackgroundEnabled(prev => !prev);
    }
    }, [isLandscapeBackgroundEnabled, instanceId, propToggleLandscapeBackground]);
  ...
  useEffect(() => {
    if (sceneRef.current) {
      ...
      // Load new background if enabled
      if (isLandscapeBackgroundEnabled) {
        // Find the renderer from the scene
        const renderer = scene.userData.renderer;
        if (renderer) {
          load16bitBackground(scene, renderer);
        }
      } else {
  ...
  const load16bitBackground = async (scene: THREE.Scene, renderer: THREE.WebGLRenderer) => {
    if (!isLandscapeBackgroundEnabled) {
      return;
    }
    ...
    try {
      ...
      const backgroundTexture = await textureLoader.loadAsync('/16bit.jpg');
      ...
      // Create new texture from darkened canvas
      const darkenedTexture = new THREE.CanvasTexture(canvas);
      
      // Configure texture for panoramic image
      darkenedTexture.wrapS = THREE.RepeatWrapping; // Horizontal wrapping for panoramic coverage
      darkenedTexture.wrapT = THREE.ClampToEdgeWrapping; // Vertical clamping to prevent stretching
      darkenedTexture.repeat.set(1, 1); // Full scale horizontal repeat
      
      // Create a smaller semi-sphere cut from top and bottom (like a cylinder) - much closer to avatars
      const sphereGeometry = new THREE.SphereGeometry(8, 32, 16, 0, Math.PI * 2, Math.PI * 0.25, Math.PI * 0.5);
      // Parameters: radius, widthSegments, heightSegments, phiStart, phiLength, thetaStart, thetaLength
      // phiStart: 0, phiLength: 2π (full horizontal circle)
      // thetaStart: π/4 (45° from top), thetaLength: π/2 (90° range) - cuts top and bottom
      // Radius reduced from 15 to 8 to make cylinder much smaller and closer
      ...
      // Add ambient lighting for better avatar visibility against darker background
      ...
      console.error(`[${instanceId}] Error loading 360-degree background:`, error);
      // Fallback to HDR if 16bit fails
      await loadHDREnvironment(scene, renderer);
    }
  };

  // Load HDR environment
  const loadHDREnvironment = async (scene: THREE.Scene, renderer: THREE.WebGLRenderer) => {
    try {
      const rgbeLoader = new RGBELoader();
      const hdrTexture = await rgbeLoader.loadAsync('/models/scenesandobj/qwantani_night_2k.hdr');
      
      // Set up environment map
      hdrTexture.mapping = THREE.EquirectangularReflectionMapping;
      scene.environment = hdrTexture;
      scene.background = null; // Remove background, keep environment for lighting
      ...
      console.error(`[${instanceId}] Error loading HDR environment:`, error);
      // Fallback: Add basic lighting if HDR fails
  ...
  const loadVRMModel = async (modelUrl: string, scene: THREE.Scene) => {
    try {
      ...
      // Only remove current model after new one is successfully loaded
      if (currentVrm.current) {
        scene.remove(currentVrm.current.scene);
      }
      ...
      toast({
        title: "Load Error",
        description: "Failed to load the VRM model. Keeping current model.",
        variant: "destructive",
      });
      
      // Restore the previous model if it exists
      if (currentVrm.current) {
        scene.add(currentVrm.current.scene);
        }
    }
  };
  ...
  // Frame counter for optimized processing
  let processingFrameCount = 0;
  
  // Tracking pause mechanism for keyboard animations
  const trackingPausedRef = useRef(false);
  const trackingPauseTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  
  // Function to pause tracking for keyboard animations
  const pauseTrackingForAnimation = useCallback((duration: number = 2000) => {
    // Clear any existing timeout
    if (trackingPauseTimeoutRef.current) {
      clearTimeout(trackingPauseTimeoutRef.current);
    }
    
    // Pause tracking
    trackingPausedRef.current = true;
    // Resume tracking after duration
    trackingPauseTimeoutRef.current = setTimeout(() => {
  ...
  const animateVRM = (vrm: VRM, results: any) => {
      ...
      // Skip tracking if paused for keyboard animations
      ...
      try {
        ...
        // Animate Pose - optimized with frame skipping
        if (pose2DLandmarks && processingFrameCount % 2 === 0) { // Process every 2nd frame
        ...
        // Animate Hands - optimized with frame skipping
        if (leftHandLandmarks && processingFrameCount % 3 === 0) { // Process every 3rd frame
        ...
        if (rightHandLandmarks && processingFrameCount % 3 === 0) { // Process every 3rd frame
  ...
  useEffect(() => {
    ...
    const setupCamera = async () => {
      try {
        ...
        .map(track => ({ kind: track.kind, enabled: track.enabled, readyState: track.readyState })));
    ...
    if (!HolisticConstructor) {
        ...
        .filter(key => key.toLowerCase().includes('holistic')));
    ...
    try {
        ...
        holistic.setOptions({
             modelComplexity: 1,  // Reduced complexity for better performance
             smoothLandmarks: true,  // Enable smoothing for more fluid tracking
             minDetectionConfidence: 0.1,  // Lower threshold for faster detection
             minTrackingConfidence: 0.1,  // Lower threshold for faster tracking
             ...
             minHandDetectionConfidence: 0.2,  // Lowered threshold for better hand detection
             minHandTrackingConfidence: 0.2,  // Lowered threshold for better hand tracking
        });
        
        holistic.onResults(onResults);
        
        // Optimized MediaPipe processing for better performance
        let frameCount = 0;
        const sendToHolistic = async () => {
            if (videoElement.readyState >= 2) {
                // Process every 2nd frame for better performance (was every frame)
                if (frameCount % 2 === 0) {
  ...
  useEffect(() => {
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      // Gesture animations - pause tracking for longer duration
      if (key === 'q') {
        pauseTrackingForAnimation(3000); // 3 seconds for greeting
        performGreeting(currentVrm.current);
      } else if (key === 'w') {
        pauseTrackingForAnimation(2500); // 2.5 seconds for big wave
        performBigWave(currentVrm.current);
      } else if (key === 'o') {
        pauseTrackingForAnimation(2000); // 2 seconds for bow
        performBow(currentVrm.current);
      }
      
      // Facial expressions - pause tracking for shorter duration
      else if (key === 'd' || key === '1') {
        pauseTrackingForAnimation(1000); // 1 second for wink
        performWink(currentVrm.current);
      } else if (key === 'f' || key === '2') {
        pauseTrackingForAnimation(1500); // 1.5 seconds for surprised
        performSurprised(currentVrm.current);
      } else if (key === 'g' || key === '3') {
        pauseTrackingForAnimation(1500); // 1.5 seconds for confused
        performConfused(currentVrm.current);
      } else if (key === 'h' || key === '4') {
        pauseTrackingForAnimation(2000); // 2 seconds for excited
        performExcited(currentVrm.current);
      }
      
      // Dance moves - pause tracking for medium duration
      else if (key === 'j') {
        pauseTrackingForAnimation(2000); // 2 seconds for head bob
        performHeadBob(currentVrm.current);
      } else if (key === 'k') {
        pauseTrackingForAnimation(2500); // 2.5 seconds for shoulder shimmy
        performShoulderShimmy(currentVrm.current);
      } else if (key === 'l') {
        pauseTrackingForAnimation(3000); // 3 seconds for hip sway
  ...
  useEffect(() => {
    ...
    if (isDualAvatar) {
      if (callMode === 'normal-video') {
        ...
        camera.position.set(0.0, 1.2, 3.0); // Further back and slightly higher to see both avatars
      }
    } else {
    ...
    renderer.setSize(mount.clientWidth, mount.clientHeight);
     renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.0)); // Cap pixel ratio for performance
    ...
    }° to ${Math.round(controls.maxPolarAngle * 180 / Math.PI)}° (20° up, 20° down)`,
    ...
    const animate = (currentTime: number) => {
      ...
      // Optimized rendering - no frame rate limiting for maximum smoothness
    ...
    return () => {
      ...
      // Cleanup tracking pause timeout
      if (trackingPauseTimeoutRef.current) {
        clearTimeout(trackingPauseTimeoutRef.current);
      }
      animationManager.current.cancelAnimation(bigWaveAnimationRef);
  ...
  useEffect(() => {
    ...
    function updateAvatarPositions() {
      ...
      if (callMode === 'normal-video') {
        ...
        // Dual Avatar Mode: Side by side - only update if not already positioned
        const localPos = currentVrm.current.scene.position;
        const remotePos = remoteVrm.current.scene.position;
        
        // Only reset to default positions if avatars are at origin (0,0,0)
        if (localPos.x === 0 && localPos.y === 0 && localPos.z === 0) {
          currentVrm.current.scene.position.set(-0.6, 0, 0); // Left side
          currentVrm.current.scene.scale.setScalar(1.0); // Normal size
        }
        
        if (remotePos.x === 0 && remotePos.y === 0 && remotePos.z === 0) {
          remoteVrm.current.scene.position.set(0.6, 0, 0); // Right side
    ...
    const reloadVRM = async () => {
      try {
        ...
        if (isDualAvatar) {
          ...
          // Ensure we have a valid model URL
          const modelUrl = selectedModel || AVAILABLE_MODELS[0].url;
          await loadVRMModel(modelUrl, scene);
        }
