...
import { Play, Settings, Mic, MicOff, Video, VideoOff, Maximize2, Home, Brain, BrainCircuit, Palette } from 'lucide-react';
...
export default function PersonaPlay3DPage() {
  ...
  const handleModelSelect = useCallback((model: any) => {
    ...
    if (!model.url || typeof model.url !== 'string') {
      console.error('Invalid model URL:', model.url);
      toast({
        title: "Invalid Model",
        description: "The selected model has an invalid URL. Please try another model.",
        variant: "destructive",
      });
      return;
    }
    
    setSelectedVrmUrl(model.url);
    setShowModelSelector(false);
  }, []);
  ...
  return (
    <div className="h-screen w-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex flex-col relative">
        ...
        <div className="w-full p-2 md:p-6">
          <div className="h-full bg-gradient-to-br from-slate-900/50 to-purple-900/50 backdrop-blur-sm rounded-2xl md:rounded-3xl overflow-hidden border border-white/10 relative">
            ...
            <ThreeCanvas 
              ...
              isLandscapeBackgroundEnabled={isLandscapeBackgroundEnabled}
              toggleLandscapeBackground={toggleLandscapeBackground}
            />
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2 md:space-x-4 mx-auto md:mx-0">
              ...
              <Button
                ...
                className="h-10 w-10 md:h-12 md:w-12 rounded-full bg-white/10 hover:bg-white/20 text-white backdrop-blur-sm border border-white/20"
                title="Change Avatar"
              >
              ...
              <Button
                ...
                className={`h-10 w-10 md:h-12 md:w-12 rounded-full text-lg transition-all duration-200 ${
                  isLandscapeBackgroundEnabled 
                    ? 'bg-orange-500 hover:bg-orange-600 text-white' 
                    : 'bg-white/10 hover:bg-white/20 text-white backdrop-blur-sm border border-white/20'
                }`}
                title="Toggle Landscape Background (Experimental)"
              >
                🌅
              </Button>
      ...
      {/* Global CSS to hide Next.js development indicators */}
      <style jsx global>{`
